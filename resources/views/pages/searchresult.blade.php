...
<div class="container-fluid">
	...
	<div class="row">
		...
		<div class="col-md-10">
			<strong>Search Result For: <u><i>{{ $keyword }}</i></u> - {{ $musics->count() }} of {{ $musics->total() }} {{ count($musics) > 1? 'songs':'song' }}</strong>
			...
			<table class="table table-hover">
				...
				<tbody>
					@foreach ($musics as $music)
						<tr>
							<td>{{ $music->title }}</td>
							<td><a href="{{ route('browse_by_artist', [$music->artist->id, $music->artist->name]) }}">{{ $music->artist->name }}</a></td>
